<?php

namespace App\Http\Livewire;

use App\Models\User;
use App\Models\Floor;
use App\Models\Campus;
use Livewire\Component;
use App\Models\Building;
use App\Models\Customer;
use Livewire\WithPagination;
use App\Models\OrganizationalUnit;
use Illuminate\Foundation\Auth\Access\AuthorizesRequests;

class BuildingCustomersDetail extends Component
{
    use WithPagination;
    use AuthorizesRequests;

    public Building $building;
    public Customer $customer;
    public $campusesForSelect = [];
    public $organizationalUnitsForSelect = [];
    public $floorsForSelect = [];
    public $usersForSelect = [];

    public $selected = [];
    public $editing = false;
    public $allSelected = false;
    public $showingModal = false;

    public $modalTitle = 'New Customer';

    protected $rules = [
        'customer.full_name' => ['nullable', 'max:255', 'string'],
        'customer.email' => ['nullable', 'email'],
        'customer.phone_no' => ['nullable', 'max:255', 'string'],
        'customer.campus_id' => ['nullable', 'exists:campuses,id'],
        'customer.organizational_unit_id' => [
            'nullable',
            'exists:organizational_units,id',
        ],
        'customer.floor_id' => ['nullable', 'exists:floors,id'],
        'customer.user_id' => ['nullable', 'exists:users,id'],
        'customer.is_edited' => ['nullable', 'max:255'],
        'customer.office_num' => ['nullable', 'max:255', 'string'],
    ];

    public function mount(Building $building)
    {
        $this->building = $building;
        $this->campusesForSelect = Campus::pluck('name', 'id');
        $this->organizationalUnitsForSelect = OrganizationalUnit::pluck(
            'name',
            'id'
        );
        $this->floorsForSelect = Floor::pluck('name', 'id');
        $this->usersForSelect = User::pluck('full_name', 'id');
        $this->resetCustomerData();
    }

    public function resetCustomerData()
    {
        $this->customer = new Customer();

        $this->customer->campus_id = null;
        $this->customer->organizational_unit_id = null;
        $this->customer->floor_id = null;
        $this->customer->user_id = null;

        $this->dispatchBrowserEvent('refresh');
    }

    public function newCustomer()
    {
        $this->editing = false;
        $this->modalTitle = trans('crud.building_customers.new_title');
        $this->resetCustomerData();

        $this->showModal();
    }

    public function editCustomer(Customer $customer)
    {
        $this->editing = true;
        $this->modalTitle = trans('crud.building_customers.edit_title');
        $this->customer = $customer;

        $this->dispatchBrowserEvent('refresh');

        $this->showModal();
    }

    public function showModal()
    {
        $this->resetErrorBag();
        $this->showingModal = true;
    }

    public function hideModal()
    {
        $this->showingModal = false;
    }

    public function save()
    {
        $this->validate();

        if (!$this->customer->building_id) {
            $this->authorize('create', Customer::class);

            $this->customer->building_id = $this->building->id;
        } else {
            $this->authorize('update', $this->customer);
        }

        $this->customer->save();

        $this->hideModal();
    }

    public function destroySelected()
    {
        $this->authorize('delete-any', Customer::class);

        Customer::whereIn('id', $this->selected)->delete();

        $this->selected = [];
        $this->allSelected = false;

        $this->resetCustomerData();
    }

    public function toggleFullSelection()
    {
        if (!$this->allSelected) {
            $this->selected = [];
            return;
        }

        foreach ($this->building->customers as $customer) {
            array_push($this->selected, $customer->id);
        }
    }

    public function render()
    {
        return view('livewire.building-customers-detail', [
            'customers' => $this->building->customers()->paginate(20),
        ]);
    }
}
