...
class LeaderUserSupportsDetail extends Component
{
    use WithPagination;
    ...
    {
        $this->editing = false;
        $this->modalTitle = trans('crud.leader_user_supports.new_title');
        $this->resetUserSupportData();
    ...
    {
        $this->editing = true;
        $this->modalTitle = trans('crud.leader_user_supports.edit_title');
    ...
    public function save()
    {
        ...
        if (!$this->userSupport->leader_id) {
            $this->authorize('create', UserSupport::class);

            $this->userSupport->leader_id = $this->leader->id;
        } else {
    ...
    public function toggleFullSelection()
    {
        ...
        foreach ($this->leader->userSupports as $userSupport) {
    ...
    {
        return view('livewire.leader-user-supports-detail', [
            'userSupports' => $this->leader->userSupports()->paginate(20),
        ]);
    }
}
